function x = APRF(S,lambda,alpha,q,omegaq,omega,sigma1,alpha1,rho,sigma3)
%% Global constants and defaults

MAX_ITER =10;

%% Data preprocessing
S = gpuArray(single(S));% Transfer data to GPU


[h_input,v_input,l_input,r_input] = MyMapping(S,sigma1,alpha1);
[row, col, cha] = size(S);

[wqx,wqy,wql,wqr] = MyRTV(S,sigma3);


wxq = 1-exp(-(wqx.^2)./(omegaq^2));
wyq = 1-exp(-(wqy.^2)./(omegaq^2));
wlq = 1-exp(-(wql.^2)./(omegaq^2));%左上角权重
wrq = 1-exp(-(wqr.^2)./(omegaq^2));%右上角权重
...
wx = 1-exp(-(wqx.^2)./(omega^2));
wy = 1-exp(-(wqy.^2)./(omega^2));
wl = 1-exp(-(wql.^2)./(omega^2));%左上角权重
wr = 1-exp(-(wqr.^2)./(omega^2));%右上角权重
...
otfFx1 = psf2otf([1,-1],size2D); % equal to otfFx = psf2otf(fx, sizeI2D) where fx = [-1, 1];
otfFy1 = psf2otf([1; -1],size2D); % equal to otfFy = psf2otf(fy, sizeI2D) where fy = [-1; 1];
otfF1=psf2otf([1 0; 0 -1], size2D);
otfF2=psf2otf([0 1; -1 0], size2D);
